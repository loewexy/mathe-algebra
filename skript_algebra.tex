...
\begin{document}
	...
	\begin{theorem}
		Sei $L/k$ eine endliche Körpererweiterung und $n \coloneqq \deg [L:k]$. Dann gilt:
		\begin{enumerate}
	...
	\begin{proof}
		...
		\heading{a)} Erinnerung es gibt höchstens $[a_1: k_0]$ viele unterschiedliche $k$-Morphismen $\sigma_1: k_1 \to \overline{k}$.
		...
		\heading{b) $\Rightarrow$} Angenommen $L/k$ ist separabel. Wir wissen: die maximale Zahl von Erweiterungen existiert, falls für alle $i$ gilt $a_{i+1}$ ist separabel über $k_i$. Per Annahme: $a_{i+1}$ ist separabel über $k$.
	...
	\begin{definition}
		...
		\begin{equation*}
			\operatorname{Gal}(L/k) = \{ \text{$k$-Morphismen $L \to L$ die surjektiv, also isomorph sind} \}
		\end{equation*}
	\end{definition}

	\heading{Beobachtung:} $\operatorname{Gal}(L/k)$ ist eine Gruppe mit Einheit $Id_L$ und der Hintereinanderausführung als Gruppenverknüpfung. Die Inversen sind die Umkehrabbildungen.
	
	\textit{Diese Gruppe heißt Galoisgruppe}
	
	\heading{Variante:} Sei $k$ ein Körper, $f \in k[x]$ ein Polynom, $L$ der Zerfällungskörper. Dann bezeichne $\operatorname{Gal}(L/k)$ auch als $\operatorname{Gal}(f)$ (Galoisgruppe von $f$).
	
	\heading{Zentrale Beobachtung:} Falls $L/k$ endlich ist dann ist $\operatorname{Gal}(L/k)$ endlich und $\#\operatorname{Gal}(L/k) \leq [L: k]$.
	
	Analog
	\begin{equation*}
		\#\operatorname{Gal}(f) \leq [ \text{Zerfällungskörper von $f$}: k ] \leq (\deg f)!
	\end{equation*}
	...
	\begin{example}
		\begin{enumerate}
			...
			$\Rightarrow \operatorname{Gal}(L/k) = \{ Id, a + b\sqrt{2} \mapsto a - b\sqrt{2} \} \simeq (\setZ/2\setZ, +)$
			\item Analog $\operatorname{Gal}(\setC/\setR) = \{ Id_\setC, \text{Konjugation} \}$
			...
			$\Rightarrow \operatorname{Gal}(L/\setQ) = \{ Id_L \}$
			...
			\textit{Insgesamt:} Der Frobenius-Morphismus ist ein $\mathbb{F}_p$-Automorphismus von $k$. $F \in \operatorname{Gal}(k/\mathbb{F}_p)$.
	...
	\heading{Beobachtung:} Wir können stets $L$ in $\overline{k}$ einbetten. Jedes $\sigma \in \operatorname{Gal}(L/k)$ ist dann automatisch ein Morphismus
	\begin{equation*}
	...
	\begin{itemize}
		...
		\item Es kann passieren, dass für gegebenes $\sigma: L \to \overline{k}$, $\operatorname{Bild}(\sigma) \neq L$ ist. $\Rightarrow$ dieses $\sigma$ liefert kein Element von $\operatorname{Gal}(L/k)$.
	\end{itemize}

	\begin{definition}
		Sei $L/k$ eine Körpererweiterung. Nenne $L/k$ normal, wenn $L/k$ algebraisch ist und wenn jedes Polynom $f \in k[x] \setminus \{ 0 \}$, das in $L$ überhaupt eine Nullstelle hat, bereits über $L$ in Linearfaktoren zerfällt.
	\end{definition}
	...
	\begin{theorem}
		...
		\begin{enumerate}
			\item $L/k$ ist normal
			\item Es gibt eine Familie $(f_\lambda)_{\lambda \in \Lambda}$ von Polynomen in $k[x]$ so dass $L$ aus $k$ entsteht durch Adjunktion sämtlicher Nullstellen der $f_\lambda$ in $\overline{k}$.
			\item Jeder Körpermorphismus $\sigma: L \to \overline{k}$ hat $\operatorname{Bild}(\sigma) = L$
		\end{enumerate}
	\end{theorem}
